function R_marked_flip = integrated_corr_atk2(R_marked, row_corr_pub, marginals_public,joints_public , gamma_r,gamma_l)


%% check mendel's law validation
mendel_violation_locations = mendelslaw_attack(R_marked); % only happends at fped locations
...
[row_idx,col_idx] = find( abs(sparse(row_corr_pub) - sparse(row_corr_marked)) >0 );

fp_row = unique(  col_idx(:) );
